...
export const hideText = ({
  ...
  hideChar='_',
  excludeLength
}:HideTextOption):string => {

  if(start === 'left'){
    const lastNthText = text.substring(0, excludeLength);
    return lastNthText.padEnd( Math.max(excludeLength, text.length) , hideChar);
  }else{
    const startNthText = text.slice(-excludeLength);
    return startNthText.padStart( Math.max(excludeLength, text.length), hideChar);
  }
...
type CreateUuidOption = {
  prefix?: string;
  suffix?: string;
  length: number
} 
export const createUuid = ({prefix= '', suffix= '', length= 64}: CreateUuidOption): string => {
...
export const numberTodataSize =(size:number)=>{
  const s = ['bytes', 'KB', 'MB', 'GB', 'TB', 'PB'];
  const e = Math.floor(Math.log(size) / Math.log(1024));
  return (s.length - 1) > e 
  ? (size / Math.pow(1024, e)).toFixed(2) + " " + s[e]
  : (size / Math.pow(1024, s.length - 1)).toFixed(2) + " " + s[s.length - 1];
